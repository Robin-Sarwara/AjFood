  ...
  const fetchReview = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `/products/${id}/sortedReview?limit=5`
      );
      setReviewData(response.data);
      // Update upvotedReviews based on server data:
    ...
    if (!isUpvoted) {
      ...
      try {
        const response = await axiosInstance.delete(
          `http://localhost:9090/api/products/review/${reviewId}/remove-upvote`,
          { data: { userId } }
        );
        console.log(response.data);
        setLoading(false);
    ...
    <>
      ...
      <div className=" p-4 h-auto w-[100%] ">
        ...
        <div className="w-[100%] h-auto ">
          <span className="mt-10 text-2xl font-bold ml-2">
            Write your review
          </span>
          <div className=" flex flex-col items-center gap-4">
            <textarea
              rows={5}
              cols={40}
              className="w-[100%] rounded-md p-1 outline-none border-2 border-black"
              type="text"
              name="review"
            ...
            <button
              ...
              className="bg-sky-600 p-2 rounded-lg w-[80px] text-black hover:bg-sky-700"
            >
        ...
        <div className="w-[100%] shadow-lg rounded-lg bg-white p-2 mt-7 h-auto">
          ...
          <div className="w-[100%] flex justify-center mt-4 mb-2">
            <button onClick={handleShowAll} className="p-2 text-white bg-gray-600 rounded-lg hover:bg-gray-800 w-auto">
              Show All
            </button>
          </div>
        </div>
